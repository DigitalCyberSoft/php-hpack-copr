...
EXT_GITHUB_REPO="DigitalCyberSoft/php-hpack"
...
check_ext_version() {
    ...
    if [ -f "$EXT_VERSION_FILE" ]; then
        saved_ver=$(cat "$EXT_VERSION_FILE" | tr -d '[:space:]')
    fi
...
if check_ext_version; then
    ...
    VERSIONS_CHANGED=1
    CHANGE_SUMMARY="${CHANGE_SUMMARY}  Extension version changed (release reset to 1)\n"
fi
echo
